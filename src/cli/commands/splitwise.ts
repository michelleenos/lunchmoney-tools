...
import { money, shorten } from '../cli-utils/write-stuff.ts'
import { getLogger } from '../cli-utils/logger.ts'
import { RootProgramOpts } from '../index.ts'
import { splitwiseToLM, splitwiseToLMWithUpdates } from '../../splitwise/splitwise-to-lm.ts'
    ...
    return program
        ...
        .action(
            programWrapper(async (_opts, command) => {
                ...
                if (start && !end) end = new Date().toISOString().split('T')[0]
                ...
                if (!assetId) {
                    ...
                    logger.info(
                        `Using asset ID ${assetId} from LM_SW_ASSET_ID environment variable`
                    )
                }
