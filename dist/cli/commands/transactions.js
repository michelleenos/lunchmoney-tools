...
export const getTransactionsCommand = () => {
    ...
    return (program
        ...
        .option('-s, --start <date>', 'Start date for transactions', defaultStart.toISOString().split('T')[0])
        ...
        const { start, end, asset, tagId, plaid, reviewed, catId, writeFile, search, verbose, } = opts;
        if (verbose)
        ...
        if (search) {
            ...
            transactions = transactions.filter((t) => t.payee?.toLowerCase().includes(searchTerm));
        }
        if (writeFile) {
