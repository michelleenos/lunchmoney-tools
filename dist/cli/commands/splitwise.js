...
export const getSplitwiseGroupCommand = () => {
    ...
    return program
        ...
        .option('--group-id <id>', 'Splitwise group ID. If not provided, will use SW_GROUP_ID env var', parseInt)
        ...
        printTable(group.members.map((member) => {
            return {
                id: member.id,
...
export const splitwiseToLMCommand = () => {
    ...
    return program
        ...
        .option('--no-filter-self', 'By default, we will filter out expenses created by the current user; use this flag to include them.')
        ...
        if (handleDupes !== 'update' && handleDupes !== 'skip') {
            throw new LMError(`Unrecognized --handle-dupes option: ${handleDupes}. Must be 'update' or 'skip'.`, 'config');
        }
...
export const lmToSplitwiseCommand = () => {
    ...
    return program
        ...
        .requiredOption('-t, --tag-id <number>', 'Lunch Money tag ID to pull transactions from (required)', parseInt)
        .option('-s, --start-date <date>', 'Start date')
        .option('-e, --end-date <date>', 'End date')
        .option('--exclude-tags <string...>', 'Tag(s) to exclude, comma-separated', (val) => val.split(',').map((s) => s.trim()), ['splitwise-auto-added'])
