...
import { searchTransactions } from "./search-transactions.js";
const logger = getLogger();
const simplifyItems = (str) => {
    return str
        .split(';')
        .map((item) => item.trim())
...
export const amazonToLM = async (lunchMoney, { filters, tags = ['amazon-link'], excludeTags = ['amazon-link'], startDate, endDate, verbose = false, filePath, dryRun = false, }) => {
    if (verbose)
    ...
    for (const order of orders) {
        ...
        if (matchingTransactions.length > 1) {
            logger.warn(`❗ Found ${matchingTransactions.length} matching transactions for order with date ${order.date} and id ${order.orderId}`);
            ...
            logger.verbose(`✨ Found matching transaction for order date ${order.date} with ID ${order.orderId}`);
...
async function parseAmazonData({ filters: { to: toField, payment: paymentContains } = {}, startDate, endDate, filePath, }) {
    ...
    data.forEach((row) => {
        if (!row['order id'] || !row['order url'] || !row.total || !row.date || !row.items) {
            logger.warn(`⚠️ Skipping row because it does not contain all required fields: ${JSON.stringify(row)}`);
